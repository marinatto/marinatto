...
vector<double> MovingAverage(vector<double> &vY_AVG, vector<double> &vY, float period){
...
int main() {
    ...
    vector<double> vY_M(vY.size(),0);
    vector<double> vY_AVG(vY.size(),0);

    Modulation(vY_M,, vY, 2);
    writeFile("modulation.log", vX, vY_M);
    MovingAverage(vY_AVG, vY, 20);
